package com.joindata.demo.pangu.camunda;
 ...
 * @date 2018年2月27日 下午4:25:36
 */
@JoindataWebApp(id = "DEMO.PANGU")
@EnableSwagger("Camunda 流程引擎开发例子")
@EnableDisconf
@EnableCamunda(url = "jdbc:mysql://10.10.110.51:3306/inf_camunda?useUnicode=true&characterEncoding=utf8&zeroDateTimeBehavior=convertToNull&rewriteBatchedStatements=false", username = "inf_camunda", password = "INF_CAMUNDA@joindata")
public class App
{
    static
    {
        ...
        System.setProperty("app.version", "Camunda");

        // 这个参数可以改变默认的 8080 端口
        // 注意！在正式运行时请务必将该参数写到启动参数中：-dhttp.port=[端口号]
        // 当部署在 Linux 系统中时，该参数可以在 /var/config/[AppID]/APP_OPTS 中修改
        // 为了保证成功启动，使用随机端口
        System.setProperty("http.port", "" + NetworkUtil.nextUsableLocalPort(8090, 9000));
    }
